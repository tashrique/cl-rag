...
class GeminiEmbedding:
    ...
    def embed_content(
        ...
        try:
            ...
            # Check if the content is too large (>9500 bytes to be safe)
            if len(text_content.encode('utf-8')) > 9500:
                logger.warning(f"Content too large for embedding: {len(text_content.encode('utf-8'))} bytes")
                # Truncate the content to a safe size
                text_content = text_content[:9000]
                logger.info(f"Truncated content to {len(text_content.encode('utf-8'))} bytes")
                ...
                if hasattr(embedding_response, 'values'):
                    ...
                    if isinstance(embedding_response, dict):
                        if "values" in embedding_response and isinstance(embedding_response["values"], list):
                            ...
                            for key, value in embedding_response.items():
                                ...
                                return self._generate_fallback_vector(768)
                    # If values is callable but not a dict method, call it to get the actual values
                        ...
                        try:
                            ...
                            logger.debug("Returning fallback vector due to error")
                            return self._generate_fallback_vector(768)
                    else:
                    ...
                    if embedding_values is None:
                        logger.warning("Embedding values is None, returning fallback vector")
                        return self._generate_fallback_vector(768)
                        
                    if not hasattr(embedding_values, '__iter__'):
                        logger.warning(f"Embedding values not iterable: {type(embedding_values).__name__}, returning fallback vector")
                        return self._generate_fallback_vector(768)
                        
                    # Convert to a list of floats, replacing any invalid values with small random values
                    clean_values = []
                    try:
                        for val in embedding_values:
                            if isinstance(val, (int, float)) and val == val and val != float('inf') and val != float('-inf'):
                                ...
                                clean_values.append(random.uniform(0.0001, 0.001))
                    ...
                    if vector_length != 768:
                        ...
                        if vector_length < 100:  # Likely not a valid embedding
                            logger.error("Embedding dimension too small, returning fallback vector")
                            return self._generate_fallback_vector(768)
                    
                    # Check if all values are zeros
                    all_zeros = all(v == 0.0 for v in clean_values)
                    if all_zeros:
                        logger.warning("Generated embedding contains all zeros, replacing with fallback vector")
                        return self._generate_fallback_vector(768)
            ...
            return self._generate_fallback_vector(768)  # Return fallback vector

    def _generate_fallback_vector(self, dimension: int = 768) -> list[float]:
        """
        Generate a fallback vector with small random values to ensure it's not all zeros.
        
        Args:
            dimension: Dimension of the vector to generate
            
        Returns:
            A list of small random float values
        """
        import random
        # Generate vector with small random values (between 0.0001 and 0.001)
        return [random.uniform(0.0001, 0.001) for _ in range(dimension)]
