...
- **Pinecone-Powered Retrieval:** Leverages Pinecone for fast, semantic document retrieval in the cloud.
...
1. **Prepare the Environment File:**
   Rename `.env.example` to `.env` and update the variables accordingly. (e.g. your [Gemini API key](https://aistudio.google.com/app/apikey) and [Pinecone API key](https://www.pinecone.io/))
...
2. **Setup a Pinecone Service:**
   Make sure that Pinecone is up an running before running your script.
   You can quickly start a Pinecone instance using Docker:

   ```bash
   docker run -p 6333:6333 pinecone/pinecone
   ```
...
│   ├── pinecone_collection.py  # Pinecone collection management
