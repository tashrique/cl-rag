...
class DeepSearchRequest(BaseModel):
    """Request model for Deep Search RAG API."""
...
class DeepSearchResponse(BaseModel):
    """Response model for Deep Search RAG API."""
    answer: str
    sources: Optional[List[DeepSearchSource]] = None
...
@router.post("/query", response_model=DeepSearchResponse)
async def deep_search_query(
    request: DeepSearchRequest,
    retriever: PineconeRetriever = Depends(get_retriever),
    responder: GeminiResponder = Depends(get_responder),
):
    """
    Process a Deep Search RAG query.
    ...
    try:
        ...
        response = DeepSearchResponse(answer=answer)
        ...
        logger.exception("Unexpected error in deep search query", error=str(e))
        return DeepSearchResponse(
